...
RUN_CMD="export CONFIG=\$(cat hack/test-config.yaml) && go run main.go"
DEBUG_CMD="export CONFIG=\$(cat hack/test-config.yaml) && dlv debug ./main.go --listen=0.0.0.0:2345 --api-version=2 --output /tmp/__debug_bin --headless"
